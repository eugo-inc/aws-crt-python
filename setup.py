...
def determine_generator_args(cmake_version=None, windows_sdk_version=None):
    if sys.platform == "win32":
        ...
        vs_version_gen_str = f"Visual Studio {vs_version} {vs_year}"
...
def using_system_libs():
    ...
    return os.getenv("AWS_CRT_BUILD_USE_SYSTEM_LIBS") == "1" or not os.path.exists(
        os.path.join(PROJECT_DIR, "crt", "aws-c-common", "CMakeLists.txt")
    )
...
class awscrt_build_ext(setuptools.command.build_ext.build_ext):
    def _build_dependencies_impl(self, build_dir, install_path, osx_arch=None):
        ...
        if sys.platform == "win32":
            ...
            cmake_args.extend(
                determine_generator_args(cmake_version=cmake_version, windows_sdk_version=windows_sdk_version)
            )
        ...
        if sys.platform == "darwin":
            ...
            if macosx_target_ver is None or parse_version(MACOS_DEPLOYMENT_TARGET_MIN) > parse_version(
                macosx_target_ver
            ):
    ...
    def _build_dependencies(self):
        ...
        if is_macos_universal2() and not is_development_mode():
            ...
            copy_tree(os.path.join(build_dir, "arm64", "install", "include"), os.path.join(install_path, "include"))
...
def awscrt_ext():
    ...
    if sys.platform == "win32":
        ...
        if forcing_static_libs():
            ...
            libraries = [f":lib{x}.a" for x in libraries]
